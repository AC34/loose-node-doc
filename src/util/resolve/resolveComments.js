...
function extractComments(func_locs,files){
  for(var path in func_locs){
    ...
    func_locs[path].comments = matchCommentsAndFunctions(func_locs[path].positions,c_positions,files[path]);
  }
  return func_locs;
}
/**
...
function locateComments(str,cpositions){
  ...
  var last_pos = cpositions.length>0?cpositions[cpositions.length-1].start:0;
  //starts_at += last_pos;
  //matched
  ...
  //add positions 
  cpositions[i] = {"start":starts_at+last_pos,"end":ends_at,"comment":str.substring(starts_at,ends_at)};
